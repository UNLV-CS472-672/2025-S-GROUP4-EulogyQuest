...
sub EVENT_SAY {
  if ($text=~/hail/i) {
    quest::say("Greetings $name, Time to start on the project");
  ...
  if ($text=~/heroes/i) {
    quest::say("Many thanks to our heroes: Kharvey, JohnZ, Kirchand, Hardy, Parham Pahlavan, AHamou, Richard, Michael, Tanner.");
  }
}
